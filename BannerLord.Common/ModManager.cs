...
namespace BannerLord.Common
{
    public sealed class ModManager : IModManager
    {
        ...
        public bool OpenModsFolder(out string errorMessage)
        {
            errorMessage = default;
            try
            {
                if (string.IsNullOrEmpty(this._modulePath) || !Directory.Exists(this._modulePath))
                {
                    errorMessage = $"{this._modulePath} is invalid";
                    return false;
                }

                var info = new ProcessStartInfo
                {
                    Arguments = this._modulePath,
                    FileName = "explorer.exe",
                    WorkingDirectory = this._modulePath,
                    UseShellExecute = false
                };
                Process.Start(info);
            }
